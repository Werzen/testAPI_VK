...
@pytest.mark.parametrize(
    ...
    ids=[
        "correct user_id",
        "incorrect user_id",
        "correct user_id with additional fields",
    ],
)
def test_users_get(params, result, get_general_params, monkeypatch):
    ...
    def mock_get_users(*args, **kwargs):
        parameters = args[1]
        if (
            parameters.get("user_ids") == str(79331389)
            and parameters.get("fields") == "city,bdate"
        ):
            return MockResponse(
                {
                    "response": [
    ...
    assert sorted(received_users.items()) == sorted(
        result.items()
